...
describe('useDocumentBrowse', () => {
  ...
  it('hides the internal wiki folder from source browsing', async () => {
    ...
    const wikiWithoutTrailingSlash = { name: 'wiki', path: '/wiki', document_count: 2 }
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(response(page([reports, wiki, wikiWithoutTrailingSlash]))))

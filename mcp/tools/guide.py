...
3. **Cite corpus entries over raw sources when both exist.** Entries are reviewed, deduplicated, dated, and carry timeliness (`M1` = fast-moving); raw files are unreviewed input. Cite the entry filename in footnotes, and fall back to the raw source only for details the entry doesn't carry. Search already enforces this: hits on an imported raw source are folded into the entry (shown as `[语料条目]` with an `原文:` pointer for verification), and unvetted sources carry trust markers — `[未入库]` (not yet classified), `[已排除: reason]` (audit rejected it — do NOT build wiki content on these), `[分类失败]`. Prefer unmarked/entry results as wiki material. Entries with `timeliness: M1` expire — `search(mode="references", query="due")` lists entries due for review; when one changes, update the wiki pages that cite it.

...
**关系层五类边**(规范 §2.5),经 MCP `relate` 工具人工/由 Claude 策划维护:

| 类型 | 含义 | 方向 (source → target) |
|---|---|---|
| `is_a` 上下位 | 概念层级 | 下位 → 上位 |
| `next` 前后置 | 流程顺序 | 前置 → 后置(如 ODI备案 → 落地设立) |
| `routes_to` 路径衔接 | 路径交接 | 如 调解 → 仲裁 → 诉讼 |
| `governed_by` 归口映射 | 规则→主管部门 | 条目 → 部门页 |
| `serves` 阶段服务包 | 阶段→推荐服务 | 阶段页 → 服务/语料集 |

与脚注引用/wiki 链接(每次写入按内容**重建**)不同,关系边**跨内容重建持久保留**,
仅 `relate(action="remove")` 可删;在 `search(mode="references")` 与图谱视图中可见。
旧库(仅允许 cites/links_to 的 CHECK 约束)启动时自动迁移;hosted 见
`supabase/migrations/009_relation_types.sql`。

**复审到期驱动**:`search(mode="references", query="due")` 返回复审工作清单
(review_due 逾期 + 待复核,按到期日升序)——夜间 Routine 一次调用即得工单。

**KPI 仪表盘**(规范 §5.4):`lint` 报告与 Web 语料库页顶部同步给出
分面完备率 · 货架覆盖率 · 时效达标率 · 引用溯源率 · 待复核数
(Web 端"待复核"卡片可点击直接筛选队列)。

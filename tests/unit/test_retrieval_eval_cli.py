...
import pytest
from scripts.retrieval_eval import _load_corpus, main
...
def test_single_profile_report_is_deterministic_and_retrieves_each_case_once(profile, capsys):
    ...
    assert payload["dataset_digest"] == evaluation_dataset_digest(cases)
...
def test_default_lexical_rejects_non_finite_corpus_metadata_without_leaking_content(tmp_path, capsys):
    ...
    (tmp_path / "corpus.jsonl").write_text(
        ...
        '"path":"/private.md","title":"Synthetic","facets":{"score":NaN},'
        '"chunks":[{"chunk_index":0,"content":"private document content"}]}\n',
        encoding="utf-8",
    )

    corpus = tmp_path / "corpus.jsonl"
    with pytest.raises(ValueError, match="corpus line is invalid"):
        _load_corpus(corpus)
...
def test_default_synthetic_lexical_baseline_is_stable_and_content_free(capsys):
    ...
    assert payload == {
        "case_count": 2,
        "dataset_digest": evaluation_dataset_digest(load_cases(DATASET)),

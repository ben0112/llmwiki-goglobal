...
This contract is the Hosted execution substrate for document extraction,
graph rebuilds, current-version embeddings, upload cleanup, and server-side
RAG. See the [platform overview](overview.md) for its place in the complete
system and [shared-kernel.md](shared-kernel.md) for the write invariants that
handlers must preserve.

## Configuration and topology

Hosted mode requires `DURABLE_JOBS_ENABLED=true` and
`TUS_MULTIPART_ENABLED=true`; the removed process-local paths are not a
fallback. `JOB_HEARTBEAT_SECONDS` must remain below `JOB_LEASE_SECONDS`, and
the TUS lock/stale/session intervals are validated in increasing order. API
and worker roles use the same Postgres and Redis configuration and must be
rolled as one release, but their replica counts scale independently behind the
gateway.
...
The API commits a ledger row before dispatching its UUID. Dispatch scans repair
lost Redis delivery, workers claim only due Postgres rows, and cancellation is
recorded in the ledger and observed at lease checkpoints. A worker crash leaves
the row recoverable: the reaper fences the expired owner, persists the next
generation, and redelivers without trusting an ARQ result payload.
...
The closing durable/scaled implementation candidate is
`08892110d85c09b8d64027cd8ab468c107ee5490`. Its
[GitHub Actions run 30152538145](https://github.com/ben0112/llmwiki-goglobal/actions/runs/30152538145)
passed all six jobs and owned every test file exactly once. The required live
smoke exercised two API and two worker replicas, Redis AOF restart, API loss,
worker TERM/SIGKILL recovery, uploads, extraction, graph rebuild, and stale
owner rejection.

Later platform publication run
[30251808426](https://github.com/ben0112/llmwiki-goglobal/actions/runs/30251808426)
also passed all six jobs at
`0173f560c6fec03b87ce4f6803f663d2d6983ead`, including the expanded scaled
Compose and Postgres matrices. Operational deployment commands and incident
rollback are kept in [the self-hosting guide](../self-hosting.md).
